...
export class SuperviseurAndCE implements CanActivate {
  ...
  canActivate(): Observable<boolean> | Promise<boolean> | boolean {//il va return soit un  Observable qui sera de type boolean soit ...un observable est un objet qui emmet des infos dans le temps
    if(!this.securityService.superviseur && !this.securityService.superviseurGene && !this.securityService.superviseurAdjoint && !this.securityService.chefEquipe){
