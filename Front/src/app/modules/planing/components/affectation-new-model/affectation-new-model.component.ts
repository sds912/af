import { Locality } from './../../../../data/schema/Locality';
...
@Component({
  selector: 'app-affectation-new-model',
  templateUrl: './affectation-new-model.component.html',
  styleUrls: ['./affectation-new-model.component.sass']
})
export class AffectationNewModelComponent implements OnInit {
  ...
  ngOnInit(): void {
   this.affectationService.opened$.subscribe((val) => {
     this.isAffecting = val;
   })
    

    this.affectationService.users$.subscribe((value) => this.selectedUsers = value);
    ...
    this.affectationService.edit$.subscribe((val) => {
      this.isEditing = val;
    })

    /*
    this.planingServ.getLocalityByLevel(0).then((localities) => {
      this.localityLevel0 = localities;

      console.log(this.localityLevel0);
    })
  ...
  affecting(){
    ...
    this.isAffecting = !this.isAffecting;
  ...
  getInventaireByEse() {
    this.inventaireServ.getInventaireByEse(this.idCurrentEse).then(
      rep => {
        ...
        this.debut = this.currentInv?.debut;
        this.fin = this.currentInv?.fin;
        this.initLocalites();
  ...
  getUsers() {
    ...
    this.adminServ.getUsers().then(
      rep => {
        ...
        this.affectationService.addUserData(this.data);
